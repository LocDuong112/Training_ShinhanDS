...
public class TutorialController {
    ...
    public ResponseEntity<List<Tutorial>> getAllTutorials(@RequestParam(required = false) String title) {
        try {
            ...
            else tutorials = sampleTutorialService.findByTitleContaining(title);
    ...
    public ResponseEntity<Tutorial> getTutorialById(@PathVariable("id") Long id) {
        Optional<Tutorial> tutorial = sampleTutorialService.findById(id);
    ...
    public ResponseEntity<HttpStatus> deleteAllTutorials() {
        try {
            sampleTutorialService.deleteAll();
    ...
    public ResponseEntity<List<Tutorial>> findByPublished() {
        try {
            List<Tutorial> tutorials = sampleTutorialService.findByPublished(true);
    ...
    public ResponseEntity<Integer> countTutorial() {
        try {
            Integer numTutorial = sampleTutorialService.countMyBatis();

            if (numTutorial == null) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }

            return new ResponseEntity<>(numTutorial, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
